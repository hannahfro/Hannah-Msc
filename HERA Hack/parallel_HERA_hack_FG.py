#these are for the telescope and observation WITH PARALLELIZATION 
import numpy as np 
...
class observation(object):
	...
	def compute_vis(self,vec,psources,fg): #DO NOT RETURN
		"""
		Compute visibility from given vector
		"""
		self.compute_Amat(psources)

		if psources is not None and fg is not None: 
			...
			pass # i think this is pass, could be continue.... 
	...
	def convolve_map(self,vec,psources,fg): #DO NOT RETURN
		...
		if self.norm is not None:
			...
			self.compute_normalization(psources)
		self.compute_vis(vec,psources,fg)
