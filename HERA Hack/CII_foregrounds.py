...
class CII_fg(object):
	...
	def intensity(self):
		...
		for i in range(self.L_tot.shape[0]): 
			...
			self.I[i] = self.L_tot[i]* (1/(factor))* (10**(26)) #[10^(-26) W m^-2 hz^-1 sr^-1 = 1 Jy/sr ] CONVERT TO JY PROPERLY 


		self.T = (self.I * (sc.c**2))/(2*(self.v_0*(10**(9)))**2 * sc.k) 
		
		print(sc.h * (self.v_0*(10**(9))))
		print(self.T * sc.k)

	def fg_temperature(self):
		self.compute_fg()
