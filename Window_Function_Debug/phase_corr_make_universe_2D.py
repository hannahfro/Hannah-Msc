...
class universe(object):
	...
	def __init__(self, ps, row_npix,col_npix, Ly,Lx, mean, sigma_decorr,sign):
	...
	def make_2D_universe(self):
		...
		for i in range(self.row_npix):
			for j in range(self.col_npix): 
				...
				b = np.random.normal(0,np.sqrt((self.ps(self.kbox[i,j])*self.Lx*self.Ly)/2)) # [mk Mpc^2]
...
######## DO THE DECORR ########
		...
		if self.sigma_decorr== 0 and self.sign == 1:
			fft_box_decorr = fft_box

		elif self.sigma_decorr== 0 and self.sign == -1:
			fft_box_decorr = -fft_box

		else:
			phi = np.random.normal(0,self.sigma_decorr,size = (fft_box.shape[0],fft_box.shape[1]))
			fft_box_decorr = fft_box*(np.exp(1j*(phi)))
		...
		for i in range(self.row_npix):
			for j in range(self.col_npix):
				...
				if mirror_i == i and mirror_j == j: 
					...
					fft_box_decorr[i,j] = np.real(fft_box_decorr[i,j])
				else:
				# copy complex conjugate from mirror coordinates to current coord
					fft_box[i,j] = np.conjugate(fft_box[mirror_i,mirror_j])
					fft_box_decorr[i,j] = np.conjugate(fft_box_decorr[mirror_i,mirror_j])

	########################################################################

		self.u = np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(fft_box * (self.delta_ky*self.delta_kx)))) #[mk]
		self.u_decorr = np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(fft_box_decorr* (self.delta_ky*self.delta_kx)))) #[mk]


		self.u /= (2.*np.pi)**2
		self.u_decorr/= (2.*np.pi)**2
		...
		if self.mean is not None: 
			...
			self.universe_decorr = (np.real(self.u_decorr))
