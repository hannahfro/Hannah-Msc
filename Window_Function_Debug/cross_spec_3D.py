...
"""
	def __init__(self, data1,data2, theta_x, theta_y , freqs, rest_freq, nbins, nbins_perp):
		
		self.data1 = data1 #- np.mean(data1)#data cube in angles and frequency coordinates
		self.data2 = data2 #- np.mean(data2)
		self.theta_x = theta_x# angular scale in the x direction RADS
		self.theta_y = theta_y #angular scale in the y direction RADS
		self.freqs = freqs*(1e6) #array of all the frequencies that were observed Hz

		self.rest_freq = rest_freq*(1e6) # Hz
		self.nbins = nbins # number of bins, 
		self.nbins_perp = nbins_perp # number of bins, 

		self.y_npix = self.data1.shape[0]
		self.x_npix = self.data1.shape[1]
		self.freq_npix = self.data1.shape[2]
		...
		if self.freq_npix % 2 == 0 :
			...
			self.mid_freq = self.freqs[(self.freq_npix//2 + 0.5)]
		...
		self.delta_thetay = (self.theta_y/self.data1.shape[0]) # size of y pixel
		self.delta_thetax = (self.theta_x/self.data1.shape[1]) # size of x pixel
		self.delta_freq = (max(self.freqs) - min(self.freqs))/self.data1.shape[2]


	def cosmo_FFT3(self): 
		''' computes the fourier transform of a 2D field of mean 0'''

		self.volume_element = (self.delta_thetax*self.delta_thetay*self.delta_freq)*(((2.99792e5) *((1+self.z)**2) * ((cosmo.comoving_distance(self.z).value)**2))/(cosmo.H0.value * cosmo.efunc(self.z) * self.rest_freq))
		self.fft_data1 = np.fft.fftshift(np.fft.fftn(np.fft.ifftshift(self.data1*self.volume_element)))# [mK rads^2 Hz] This is "observer" fourier transform

		self.fft_data2 = np.fft.fftshift(np.fft.fftn(np.fft.ifftshift(self.data2*self.volume_element)))
	
		self.ps_data = (np.conj(self.fft_data1))*self.fft_data2 # [mK rads^2 Hz]^2
...
## compute k_par, k_perps
	...
	def compute_2D_pspec(self):
		...
		for l in range(len(self.k_par)):
			...
			for i in range(self.data1.shape[0]) : #theta_y direction
				for j in range(self.data1.shape[1]): #theta_x direction
					kx = ((j-(self.data1.shape[0]/2))*self.delta_kx)#need to multiply by kdelta to get your k units
					ky = ((i-(self.data1.shape[1]/2))*self.delta_ky)
	...
	def compute_1d_from_2d(self):
		...
		for i in range(len(self.k_par)):
			for j in range(self.nbins_perp):
				kpar = (i - (self.data1.shape[2]/2))*self.delta_kz
	...
	def compute_1D_pspec(self):# this is very confusing, i am not sure how to do this now that k_perp is not from - to +
		...
		for l in range(self.data1.shape[2]):
			for i in range(self.data1.shape[0]) : #theta_y direction
				for j in range(self.data1.shape[1]): #theta_x direction
					kx = ((j-(self.data1.shape[1]/2))*self.delta_kx) #need to multiply by kdelta to get your k units
					ky = ((i-(self.data1.shape[0]/2))*self.delta_ky)
					kz = ((l - (self.data1.shape[2]/2))*self.delta_kz)
