...
class universe(object):
	...
	def __init__(self, ps, row_npix,col_npix, Ly,Lx, mean, sigma_decorr,sign, nbins):
...
######## DO THE DECORR ########
			...
			for j in range(fft_box.shape[1]):
				...
				for k in range(len(bin_edges)-1): #make sure that you speed this up by not considering already binned ps's
					if bin_edges[k] < kmag <= bin_edges[k+1]: 
						...
						if self.sigma_decorr[k]== 0 and self.sign == 1:
							fft_box_decorr[i,j] = fft_box[i,j]

						elif self.sigma_decorr[k]== 0 and self.sign == -1:
							fft_box_decorr[i,j] = -fft_box[i,j]

						else:
							
							phi = np.random.normal(0,self.sigma_decorr[k])
							fft_box_decorr[i,j] = fft_box[i,j]*(np.exp(1j*(phi)))
	...
	########################################################################
		...
		self.u_decorr = np.fft.fftshift(np.fft.fftn(np.fft.ifftshift(fft_box_decorr* (self.delta_ky*self.delta_kx)))) #[mk]
