...
import time
import HERA_FG_PS ##make sure it is in directory
import FG_pygsm
...
HERA = HERA_FG_PS.telescope(hera_bls, latitude=-30, channel_width=97800, beam_width=10, beam = 'gaussian')
...
if myID == master:
    ...
    a_file = open("obs_Hnoise_fg_ps_774.txt", "w")
    print('made the file')
    ...
    while (complete == False):
        ...
        if tag == 0:
            ...
            obs = HERA_FG_PS.observation(telescope = HERA, n_days = 42, freq = obs_freq, delta_t = 0.01 ,corners = acorner, beam_sigma_cutoff=1, sky_shape = (npix_row,npix_col), norm = norm , pbeam = pbeam)
##FG stuff            
            ...
            sky_map = np.real(obs.map + obs.noise/300)[:npix_col*npix_row]# observe the sky 
