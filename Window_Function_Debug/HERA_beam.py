...
class observation(object):
	...
	def observable_coordinates(self):
		...
		if max_corner[0]*(np.pi/180.) < min_obsbound or min_corner[0]*(np.pi/180.) > max_obsbound:
			...
			psource_pos = np.array([[2.09,min(self.position[:,1])-0.02],[2.110,max(self.position[:,1])+0.03],[min(self.position[:,0])-0.044,4.79]])
			self.position = np.vstack((self.position,psource_pos))
	...
	def compute_beam_wrong(self,psource_beam): #DO NOT RETURN
		"""
		Compute Primary Beam assuming all antenna has an identical beam with the fixed sky.
		"""
		beam_wrong = 0.9 # FWHM in degrees
		beam_wrong *= np.pi / 180. # FWHM in radians
		...
		if self.beam == 'gaussian':
			...
			for i in range(self.Nt): #compute the elements of pbeam
				primary[i] = np.exp(-((self.position[:,1]-phis[i])**2 +(self.position[:,0]-co_lat)**2) / float((beam_wrong)**2))# 2D gaussian beam (N_position,2)
		...
		self.pbeam_wrong = np.vstack([primary] * self.Nbl) #to replace the for loop pbeam


	def compute_Amat_wrong(self,psource_data, psource_beam): #DO NOT RETURN
		"""
		Compute A matrix
		"""
		if self.times is not None:
		...
		self.compute_beam_wrong(psource_beam)
		...
		if self.primary_beam == True: #the compute Amat with pbeam 
			...
			self.Amat_wrong = self.pbeam_wrong*exponent*self.delta_theta*self.delta_phi
			
		elif self.primary_beam == False: # this computes Amat without the pbeam
			self.Amat_wrong = exponent*self.delta_phi*self.delta_theta
		else: 
	...
	def compute_vis(self,vec,psource_data, psource_beam): #DO NOT RETURN
		...
		psource_flux = np.array([1e3,5e3,7e3])#K
		vec_ps = np.hstack((vec,psource_flux))
		self.Adotx = np.dot(self.Amat, vec_ps) 

	def compute_normalization(self,psource_data, psource_beam): #DO NOT RETURN

		 ## compute [A^*t N^{-1} A]^{-1}
		self.compute_Amat_wrong(psource_data, psource_beam)
						  
		AtA = ((np.conj(self.Amat_wrong)).T).dot(self.invN).dot(self.Amat_wrong) 
	...
	def generate_map_noise(self,psource_data, psource_beam):#DO NOT RETURN
		...
		if self.normalization == True: #uses the normalization in the estimator
			...
			self.noise = np.dot(self.norm,(np.conj(self.Amat_wrong)).T).dot(self.invN).dot(self.my_noise)
	...
	def convolve_map(self,vec,psource_data,psource_beam ): #DO NOT RETURN
		...
		if self.normalization == True: #uses the normalization in the estimator
			...
			self.map = np.dot(self.norm,((np.conj(self.Amat_wrong)).T).dot(self.invN).dot(self.Adotx))
			...
			self.map = ((np.conj(self.Amat_wrong)).T).dot(self.invN).dot(self.Adotx)
